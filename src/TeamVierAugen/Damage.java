...
public class Damage
{
    ...
    private static final int SPEED_CONDITIONAL_DAMAGE = 5;
            ...
            throws Exception
    {
        ...
        switch(usedSkill)
        {
            case ROCK_THROW:
                ...
                else if(enemyPlayer.getSkillRechargeTime(Skills.ROCK_THROW) > 0)
                    damage = INTELLIGENCE_CONDITIONAL_DAMAGE_LOW;
                break;
            case PAPER_CUT:
                if(enemyPlayer.getSkillRechargeTime(Skills.ROCK_THROW) > 0)
                    damage = INTELLIGENCE_CONDITIONAL_DAMAGE_HIGH;
                else if(enemyPlayer.getSkillRechargeTime(Skills.PAPER_CUT) > 0)
                    damage = INTELLIGENCE_CONDITIONAL_DAMAGE_LOW;
                break;
            case SCISSORS_POKE:
                if(enemyPlayer.getSkillRechargeTime(Skills.PAPER_CUT) > 0)
                    damage = INTELLIGENCE_CONDITIONAL_DAMAGE_HIGH;
                else if(enemyPlayer.getSkillRechargeTime(Skills.SCISSORS_POKE) > 0)
                    damage = INTELLIGENCE_CONDITIONAL_DAMAGE_LOW;
                break;
            default:
                throw new Exception("Invalid skill type!");
