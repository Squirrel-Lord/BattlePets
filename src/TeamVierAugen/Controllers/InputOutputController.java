...
public class InputOutputController
{
    ...
    private final int MIN_SEED = 3; // temp maybe change???
    ...
    public int getRandomSeed()
    {
        ...
        {
            System.out.println("Please enter the RNG seed number (MIN TEMP3)");
            temp = readInteger();
            inputReader.nextLine();
            while(temp < MIN_SEED)
            {
                System.out.println(temp + " is not valid, please enter a valid RNG seed number (MIN TEMP3)");
                temp = readInteger();
                inputReader.nextLine();
            }
            return temp;
