...
public class RoundController {
    ...
    public List<Playable> completeOneRound() {
        ...
        try {
            ...
            damagePlayers(playerList, round.getPlayerDamages());
        ...
        for (int i = 0; i < playerStatuses.size(); i++)
        {
            if (playerSkillChoices.get(playerStatuses.get(i)) == Skills.ROCK_THROW ||
                    playerSkillChoices.get(playerStatuses.get(i)) == Skills.PAPER_CUT ||
                    playerSkillChoices.get(playerStatuses.get(i)) == Skills.SCISSORS_POKE)
            {
                playerStatuses.get(i).setRechargeTime(playerSkillChoices.get(playerStatuses.get(i)),
                        SKILL_RECHARGE_TIME_1);
            }
            else
            {
                playerStatuses.get(i).setRechargeTime(playerSkillChoices.get(playerStatuses.get(i)),
                        SKILL_RECHARGE_TIME_2);
            }
        }
     ...
     */
    private void damagePlayers(List<Playable> players, Map<Playable, Damage> playerDamages)
    {
        for (int i = 0; i < players.size(); i++)
        {
            ioController.displayDamage(playerDamages.get(players.get(i)), players.get(i));
            players.get(i).updateHp(playerDamages.get(players.get(i)).calculateTotalDamage());
    ...
    private void checkSleepScent(List<Playable> playerList)
    {
        ...
        while (petsGoingToSleep.size() > 0)
        {
            ...
            for (int i = 0; i < playerList.size(); i++)
            {
                ...
                {
                    PlayerInstance player = (PlayerInstance) playerList.get(i);
